...
The max log-likelihood ratio is actually `r max(llike_ran$logLR)`, but the estimate of $\pi_0$ in this scenario is `r pi0_max`. In general, the estimates of $\pi_0$ for all scenarios that have a log-likelihood ratio greater than 0 is
```{r}
pi0_mat$succ_random[llike_ran$logLR > 0]
boxplot(pi0_mat$succ_random[llike_ran$logLR > 0])
```

Since I'm starting at random locations, it's clear that I'm just getting stuck at local maxima in the `r sum(pi0_mat$succ_random[llike_ran$logLR > 0] < 0.99)` cases (LR > 1 but $\hat{\pi}_0$ < 0.99) where $\hat{\pi}_0$ is not really close to 1.
