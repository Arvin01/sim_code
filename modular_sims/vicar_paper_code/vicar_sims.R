...
one_rep <- function(new_params, current_params) {
    ...
    ash_list <- lapply(method_list[-c(8, 10)], ash_wrap)
...
## these change
Nsamp_seq  <- c(5, 10, 25, 50)
nullpi_seq <- c(0.5, 0.75, 0.9, 0.95, 1)
ncontrol <- c(100, 1000)
...
colnames(sout)[27:36]
auc_pvalues   <- cbind(par_vals, sout[, 1:10])
auc_ash       <- cbind(par_vals, sout[, 11:18])
pi0hat_ash    <- cbind(par_vals, sout[, 19:26])
pi0hat_qvalue <- cbind(par_vals, sout[, 27:36])
fdp_mat       <- cbind(par_vals, sout[, 37:46])
