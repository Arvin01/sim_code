...
one_rep <- function(new_params, current_params) {
    return_vec <- tryCatch(expr = {
        ...
        method_list$catenc <- cate_nc(Y = Y, X = X, num_sv = num_sv,
                                      control_genes = control_genes,
                                      calibrate = TRUE)
        method_list$ruvb   <- ruvb(Y = Y, X = X, num_sv = num_sv,
                                   control_genes = control_genes)
...
save(sout, file = "general_sims8.Rd")
mse_mat <- cbind(par_vals, sout[, 1:6])
auc_mat <- cbind(par_vals, sout[, 7:12])
cov_mat <- cbind(par_vals, sout[, 13:18])
write.csv(mse_mat, file = "mse_mat8.csv", row.names = FALSE)
write.csv(auc_mat, file = "auc_mat8.csv", row.names = FALSE)
write.csv(cov_mat, file = "cov_mat8.csv", row.names = FALSE)
