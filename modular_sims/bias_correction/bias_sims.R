...
one_rep <- function(new_params, current_params) {
    ...
    method_list$vruv4_badjust <- vruv4(Y = Y, X = X, num_sv = num_sv,
...
auc_pvalues   <- cbind(par_vals, sout[, 1:11])
auc_ash       <- cbind(par_vals, sout[, 12:20])
pi0hat_ash    <- cbind(par_vals, sout[, 21:29])
pi0hat_qvalue <- cbind(par_vals, sout[, 30:40])
fdp_mat       <- cbind(par_vals, sout[, 41:51])
