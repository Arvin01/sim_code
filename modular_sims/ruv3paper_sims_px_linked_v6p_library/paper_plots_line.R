...
good_names <- c("OLS", "RUV2", "RUV2c", "RUV3",
                "RUV4", "RUV4c", "CATE", "CATEv", "RUVB")
...
longcov$Lower <- longcov$Mean - 2 * longcov$SD / sqrt(200)
longcov$Upper <- longcov$Mean + 2 * longcov$SD / sqrt(200)
...
pdf(file = "coverage_lines_5.pdf", family = "Times", height = 3.2, width = 6.5)
print(pl)
dev.off()
...
pdf(file = "coverage_lines.pdf", family = "Times", height = 7.5, width = 6.5)
print(pl)
dev.off()
...
long_diff$lower <- long_diff$mean - 2 * long_diff$sd / sqrt(200)
long_diff$upper <- long_diff$mean + 2 * long_diff$sd / sqrt(200)
...
pl <- ggplot(final_diff, mapping = aes(x = Nsamp, y = Mean, color = Method, lty = Method)) +
    ...
    xlab("Sample Size") +
    theme(strip.background = element_rect(fill="white"))
pdf(file = "diff_lines_5.pdf", family = "Times", height = 3.2, width = 6.5)
print(pl)
dev.off()
...
pdf(file = "diff_lines.pdf", family = "Times", height = 7.5, width = 6.5)
print(pl)
dev.off()
