...
one_rep <- function(new_params, current_params) {
    ...
    ## DESeq normalization ----------------------------------------------------------
    countmat <- as.matrix(d_out$input$counts + 1)
    size_factors <- DESeq::estimateSizeFactorsForMatrix(countmat)
    Y <- t(log2(sweep(countmat, 2, 1 / size_factors, `*`))) ## need to divide

    ## edgeR normalization
    ## countmat <- as.matrix(d_out$input$counts + 1)
    ## size_factors <- edgeR::calcNormFactors(countmat)
    ## Y <- t(log2(sweep(countmat, 2, size_factors, `*`))) ## need to multiply
