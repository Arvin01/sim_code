...
num_sv_seq <- readRDS("./output/ruvbout_q75/num_sv.Rds")
...
for(tissue_index in 1:length(tissue_vec)) {
    ...
    ruvbout  <- readRDS(paste0("./output/ruvbout_q75/ruvbout_", current_tissue, ".Rds"))
    olsout   <- ols(Y = t(dat$Y), X = X)
    ruv2out  <- ruv::RUV2(Y = t(dat$Y), X = X[, 2, drop = FALSE], ctl = dat$ctl,
                          k = num_sv, Z = X[, -2, drop = FALSE])
    ruv2cout <- ruv::variance_adjust(ruv2out)
    ruv3out  <- vicar::ruv3(Y = t(dat$Y), X = X, ctl = dat$ctl, cov_of_interest = 2,
                            k = num_sv, include_intercept = FALSE)
    ruv4out  <- ruv::RUV4(Y = t(dat$Y), X = X[, 2, drop = FALSE], ctl = dat$ctl,
                          k = num_sv, Z = X[, -2, drop = FALSE])
    ruv4cout <- ruv::variance_adjust(ruv4out)
    cateout  <- cate::cate.fit(X.primary = X[, 2, drop = FALSE],
...
pdf(file = "propk_q75.pdf", family = "Times", height = 5, width = 6.5)
print(pgd)
dev.off()
